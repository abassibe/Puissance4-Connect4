...
/*   Updated: 2017/03/05 18:36:13 by abassibe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		easy_counter_win_h(t_easy *verif)
{
	...
	j = verif->pos_j;
	if (i > 0)
		if (g_connect.grid[i - 1][j] == 0 && (j + 1 == g_connect.input_h ||
					g_connect.grid[i - 1][j + 1] != 0))
		{
	...
	if (i < g_connect.input_w - 3)
		if (g_connect.grid[i + 3][j] == 0 && (j + 1 == g_connect.input_h ||
					g_connect.grid[i + 3][j + 1] != 0))
		{
...
int		easy_counter_win_g(t_easy *verif)
{
	...
	i = verif->pos_i;
	if (i > 0 && j > 0)
		if (g_connect.grid[i - 1][j - 1] == 0 && g_connect.grid[i - 1][j] != 0)
		{
			if (i - 1 < 0 || j - 1 < 0)
				return (0);
			g_connect.insert_c = i - 1;
	...
	if (i < g_connect.input_w - 3 && j < g_connect.input_h - 3)
		if (g_connect.grid[i + 3][j + 3] == 0 &&
				g_connect.grid[i + 3][j + 4] != 0)
		{
...
int		easy_counter_win_d(t_easy *verif)
{
	...
	if (i > 2 && j < g_connect.input_h - 3)
		if (g_connect.grid[i - 1][j + 1] == 0 &&
				g_connect.grid[i - 1][j + 2] != 0)
		{
